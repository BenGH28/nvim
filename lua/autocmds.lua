...
-- make sure tmux status is always on top while in neovim
if vim.fn.has("unix") then
  local has_tmux = os.execute("tmux -V") == 0
  -- Check if tmux is running
  local in_tmux = io.popen("pgrep tmux"):read("*a") ~= ""
  if has_tmux and in_tmux then
    local tmux_status_position = augroup("tmux-status-position", opts)
    au("VimEnter", {
      pattern = "*",
      callback = function()
        vim.fn.jobstart("tmux set status-position top")
      end,
      group = tmux_status_position,
    })
    au("VimLeavePre", {
      pattern = "*",
      callback = function()
        vim.fn.jobstart("tmux set status-position bottom")
      end,
      group = tmux_status_position,
    })
  end
